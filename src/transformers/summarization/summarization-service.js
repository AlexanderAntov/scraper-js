...
export default class SummarizationService {
    summarize(title, content, callback, sentencesDictionary) {
        ...
        this._getSentencesRanks(content, dictionary => {
            splitContentToParagraphs(content, paragraphs => {
                ...
                paragraphs.forEach(paragraph => {
                    this._getBestSentence(paragraph, dictionary, sentence => {
                        if (sentence) {
    ...
    _getSentencesRanks(content, callback, sentencesDictionary) {
        if (sentencesDictionary !== undefined) {
            callback(sentencesDictionary);
            return;
        } else {
            sentencesDictionary = {};
        }
    
        this._splitContentToSentences(content, sentences => {
            let sentencesLength = sentences.length,
                zeroToNRange = range(0, sentencesLength);
    
            let values = [],
                temp = [];
    
            zeroToNRange.forEach(() => {
                temp = [];
                zeroToNRange.forEach(() => {
                    temp.push(0);
                });
            ...
            zeroToNRange.forEach(i => {
                zeroToNRange.forEach(j => {
                    this._sentencesIntersection(sentences[i], sentences[j], (error, intersection) => {
                        if (error) {
                            throw error;
                        }
            ...
            some(zeroToNRange, i => {
                ...
                this._formatSentence(sentences[i], formattedSentence => {
                    sentencesDictionary[formattedSentence] = score;
                });
            });
    ...
    _getBestSentence(paragraph, sentencesDictionary, callback) {
        this._splitContentToSentences(paragraph, sentences => {
    ...
    _sentencesIntersection(sentence1, sentence2, callback) {
        ...
        const splicePivot = ((words1.length + words2.length) / 2);
        callback(false, this._getWordsIntersection(words1, words2).splice(0, splicePivot).length);
    }

    _getWordsIntersection(sentence1, sentence2) {
        let sentence1index = 0,
            sentence2index = 0,
            result = [];
    
        while (sentence1index < sentence1.length && sentence2index < sentence2.length) {
            if (sentence1[sentence1index] < sentence2[sentence2index]) {
                sentence1index++;
            } else if (sentence1[sentence1index] > sentence2[sentence2index]) {
                sentence2index++;
            } else {
                result.push(sentence1[sentence1index]);
                sentence1index++;
                sentence2index++;
            }
        }
    ...
    _splitContentToSentences(content, callback) {
        if (content.indexOf('.') === -1) {
            return callback(false);
        }
    
        callback(sbd.sentences(content, { newline_boundaries: true }) || []);
    }
}