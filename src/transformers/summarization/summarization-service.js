...
export default class SummarizationService {
    summarize(title, content, sentencesDictionary) {
        ...
        paragraphs.forEach(paragraph => {
            const sentence = this._getBestSentence(paragraph, dictionary);
            if (sentence) {
    ...
    _getSentencesRanks(content, sentencesDictionary) {
        ...
        const sentences = this._splitContentToSentences(content);
        ...
        zeroToNRange.forEach(i => {
            zeroToNRange.forEach(j => {
                const intersection = this._sentencesIntersection(sentences[i], sentences[j]);
        ...
        some(zeroToNRange, i => {
            ...
            sentencesDictionary[this._formatSentence(sentences[i])] = score;
        });

        return sentencesDictionary;
    }

    _getBestSentence(paragraph, sentencesDictionary) {
        const sentences = this._splitContentToSentences(paragraph);
        ...
        for (index in sentences) {
            ...
            const formattedSentence = this._formatSentence(sentence);
    ...
    _formatSentence(sentence) {
        if (sentence && sentence.replace) {
            ...
            return sentence.replace(unicodeBreakoutRegex, '');
        }
        return sentence;
    }

    _sentencesIntersection(sentence1, sentence2) {
        ...
        return this._getWordsIntersection(words1, words2).splice(0, splicePivot).length;
    }
    ...
    _splitContentToSentences(content) {
